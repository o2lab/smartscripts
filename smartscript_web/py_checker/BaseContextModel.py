...
basetype = ['int','float','complex','bool','str','list','dict','set','tuple','object','type']
...
vocab_size = 20000
lr = 0.0001
model_folder = "/home/smartscript/smartscript_web/py_checker/BaseContext"
MAX_SEQ_LEN = 512
saved = False
if not os.path.exists(model_folder):
        ...
        lossWriter = open("./Loss.txt","w",encoding='utf-8')

elif os.path.exists(model_folder+'/model_type/TypeModel.ckpt'):
    savedModel = torch.load(model_folder+'/model_type/TypeModel.ckpt',map_location=device)
    trainWriter = open("./TrainLog.txt","a",encoding='utf-8')
    validWriter = open("./ValidLog.txt","a",encoding='utf-8')
    testWriter = open("./TestLog.txt","a",encoding='utf-8')
    lossWriter = open("./Loss.txt","a",encoding='utf-8')
    saved = True
    print('Model Loaded.')
# In[ ]:
try: 
...
spmPath = "/home/smartscript/smartscript_web/py_checker/BaseContext/"
# dataPath = "/home/Think/pkldata/"
# dataPath = "/mnt/data/newdata/dict1024.pkl"
dataPath = ''
Type500 = "/home/smartscript/smartscript_web/py_checker/BaseContext/"
...
class BugDetector(nn.Module):
    def __init__(self,
        ...
        self.cls = AttnClassifier('dot', self.embedding, model_size, 11, decoder_n_layers, p_dropout)
...
def main():
    ...
    if saved:
        detector = BugDetector(savedModel['config']['vocab_size'], savedModel['config']['max_src_seq_len'], model_size,
                            savedModel['config']['dropout'])
        detector.load_state_dict(savedModel['model'])
