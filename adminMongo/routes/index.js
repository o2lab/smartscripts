...
router.get('/app/:conn/:db/:coll/edit/:doc_id', function (req, res, next){
    ...
    common.get_sidebar_list(mongo_db, req.params.db, function(err, sidebar_list){
	    ...
	    var diff_srcs = {};
            _.forOwn(result.doc, function(value, key) {
                if (key) {
                    if (key.toString() === 'original_code') {
                        diff_srcs['original_code'] = value;
                    } else if (key.toString() === 'mutated_code') {
                        diff_srcs['mutated_code'] = value;
                    }
                }
            });
	    var diff_srcs_json = null;
            if (diff_srcs['original_code']) {
                diff_srcs_json = bsonify.stringify(diff_srcs, null, '    ');
            }
	
	    var single_src = bsonify.stringify(result.doc, null, '    ');//null;
            // _.forOwn(result.doc, function(value, key) {
            //     // if (key) {
